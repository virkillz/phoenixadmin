defmodule PhoenixadminWeb.ActivityControllerTest do
  use PhoenixadminWeb.ConnCase

  alias Phoenixadmin.Logging

  @create_attrs %{activity: "some activity"}
  @update_attrs %{activity: "some updated activity"}
  @invalid_attrs %{activity: nil}

  def fixture(:activity) do
    {:ok, activity} = Logging.create_activity(@create_attrs)
    activity
  end

  describe "index" do
    test "lists all activity", %{conn: conn} do
      conn = get conn, activity_path(conn, :index)
      assert html_response(conn, 200) =~ "Listing Activity"
    end
  end
